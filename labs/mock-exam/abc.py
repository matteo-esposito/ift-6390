...
def find_bins(input_list, k):
    ...
    for idx, i in enumerate(input_list):
        if len(bins) > k: break;
        if idx==0:
            bins.append(int(input_list[idx-1])-1)
        else:
            bins.append(int(input_list[idx])-1)
    return bins

#     hist, bin_edges = np.histogram(input_list, bins=k)
...
print(even_odd_ordered([1, 2, -3, 4, 7, 4, -6, 3, -1]))
...
def data_normalization(X):
    ...
    for row in X:
        ...
        data = row[:len(row)-1]
        print(data)
        norm_data.append((data - np.mean(data))/(np.std(data)))
        print(norm_data)
...
print(data_normalization([[1., -1., 2., 1], [2., 0., 0., 2], [0., 1., -1., 3]]))
...
def entropy(p):
    ...
    """
    if np.sum(p) == 1 and all(i >= 0 for i in p):
...
entropy([0.6, 0.1, 0.25, 0.05])


# In[94]:


entropy([0.3, 0.75, -0.3, 0.25])
...
class NearestCentroidClassifier:
    ...
    def fit(self, X, y):  # question A
        ...
        for cla in range(self.k):
            x_data = X[y == cla]
            self.centroids[cla] = int(np.mean(x_data))
    ...
    def predict(self, X):  # question B
        ...
        for point in X:
            for i in range(self.k):
                diffs.append(np.sum(self.centroids[i] - point) ** 2)
                print(diffs.index(min(diffs)))
            preds.append(diffs.index(min(diffs)))
